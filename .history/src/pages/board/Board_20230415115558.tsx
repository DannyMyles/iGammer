...
const Board = ({ tilesPerRow, width, imageUrl }: BoardProps) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (tiles.length) {
        setTiles(
          tiles.map((t: TitleState, i: number) => {
            const t1 = t;
            const t2 = tiles[Math.floor(Math.random() * tiles.length - 1)];
            t.left = t2.left;
            t.top = t2.top;
            t2.left = t1.left;
            t2.top = t1.top;
  ...
  return (
    ...
    >
      ...
      {tiles.map((tile, i) => (
        <img
          key={i + 1}
          style={{ ...tile, position: "absolute", background:"red" }}
          alt="test"
          src={tile.url}
          onClick={() => move(tile)}
        />
