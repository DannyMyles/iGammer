...
const Board = ({ width, imageUrl }: BoardProps) => {
  // 
  ...
  function randomizeTiles(mts:TileState[]){  
    ...
    const tr1 = Math.floor(Math.random() * mts.length);
      ...
      return mts.map((n, i) => {
        ...
        if (i % board.tilesPerRow === 0) {
          if (i >= board.tilesPerRow) {
  ...
  function init(){
    const tilesPerRow = board.tilesPerRow;
    // debugger;
    const bw = Math.floor(width / tilesPerRow) * tilesPerRow;
