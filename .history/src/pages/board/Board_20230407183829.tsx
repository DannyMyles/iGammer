...
const Board = ({ tilesPerRow, width, imageUrl }: BoardProps) => {
  ...
  // 
  const [tileWidth, SetTileWidth] = useState<number>( width / tilesPerRow );
  ...
  return (
    ...
    >
      ...
      {tiles.map((tile, i)=><img key={i+1} style={{background:"red", margin:0}} alt="test" src={getTileImageUrl(tileWidth, tileWidth, tile.x, tile.y, imageUrl)} />)}
    </div>
  );
};
