...
const Board = ({ tilesPerRow, width, imageUrl }: BoardProps) => {
  ...
  function randomizeTiles(mts:TileState[]){
    ...
    const tileIndex = movableTileIndex[ Math.floor(Math.random() * movableTileIndex.length) ];
    const tile = mts.find(t=>t.tileIndex === tileIndex);
    ...
    const clickedTileIndex = mts.findIndex(t=>t.tileIndex === tile!.tileIndex);
    // if(clickedTileIndex > -1){

      setMissingTile({...tile!, x:tile!.x, y:tile!.y});

      return mts.splice(clickedTileIndex, 1, {
        ...tile!, 
        x: newPosition!.x,
        y: newPosition!.y,  
