...
const Board = ({ tilesPerRow, width, imageUrl }: BoardProps) => {
  ...
  function randomizeTiles(mts:TileState[]){
    ...
    const movableTileIndex = Math.floor(Math.random() * movableTiles.length);

    // move( mts[movableTileIndex] );

    // const indexOfMissingTile = 
  
      // const holdingTile = {...mts[movableTileIndex]}
      
      // mts[movableTileIndex] = {
      //   index: missingTile?.tileIndex!,
      //   left: `${missingTile?.x}px`,
      //   top: `${missingTile?.y}px`,
      //   url: tileImages[missingTile?.tileIndex || 0],
      //   x: missingTile?.x || 0,
      //   y: missingTile?.y || 0,
      //   tileIndex: missingTile?.tileIndex!
      // }; 
      // setMissingTile(holdingTile);
