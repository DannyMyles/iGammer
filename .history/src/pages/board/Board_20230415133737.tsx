...
const Board = ({ tilesPerRow, width, imageUrl }: BoardProps) => {
  ...
  function randomizeTiles(tiles:TileState[]){
    ...
    const interChange = ()=>{
      const tr1 = Math.floor(Math.random() * unMovedTiles.length);
      const tr2 = Math.floor(Math.random() * unMovedTiles.length);
  
      console.log(tr1, tr2);  
  
      const holdingTile = {...tiles[tr1]}
      tiles[tr1] = {...tiles[tr2]};
      tiles[tr2] = {...holdingTile}
