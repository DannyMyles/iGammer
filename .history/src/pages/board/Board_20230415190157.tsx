...
const Board = ({ tilesPerRow, width, imageUrl }: BoardProps) => {
  ...
  function randomizeTiles(mts:TileState[]){
    ...
    const movableTileIndexs = getMovableTiles();
    const selectedTileIndex = movableTileIndexs[Math.floor(Math.random() * movableTileIndexs.length)];
    move( mts[selectedTileIndex] );
