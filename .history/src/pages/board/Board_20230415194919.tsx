...
const Board = ({ tilesPerRow, width, imageUrl }: BoardProps) => {
  ...
  function randomizeTiles(mts:TileState[]){
    ...
    const tileIndex = movableTiles[ Math.floor(Math.random() * movableTiles.length) ]
    console.log(tileIndex);
  
    const tr1 = mts.findIndex(t=>t.tileIndex === tileIndex); //Math.floor(Math.random() * mts.length);
      // const tr2 = mts.findIndex(t=>t.tileIndex === missingTile?.tileIndex);
  
      const holdingTile = {...missingTile} 

      setMissingTile({...mts[tr1]}); 

      return mts.splice(tr1, 1, {
        ...mts[tr1], 
        x: holdingTile!.x,
        y: holdingTile!.y, 
        top:`${holdingTile!.y}px`, 
        left:`${holdingTile!.x}px`
      })
