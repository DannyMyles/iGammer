...
const Games:React.FC<Props>  = ({ imagePath, rows, columns }) => {
  const [tiles, setTiles] = useState<Tile[] | any>(createTiles());
  ...
  function handleClick(clickedTile: Tile): void {
    const emptyTile = tiles.find((tile:any) => tile.id === rows * columns - 1);
  ...
  const isSolved = tiles.every((tile:any) => tile.id === tile.x + tile.y * columns);
