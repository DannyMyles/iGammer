...
export function getSqueredImage(imageUrl:string, size:number){
    ...
    ctx?.drawImage(originalImage, 0, 0, size, size, size, size, size, size);

    return canvas.toDataURL();
}

export function getTileImageUrl( width:number, height:number, x:number, y:number, imageUrl:string ){
    const canvas = document.createElement('canvas');
    canvas.width = width;
    ...
    image.width = width;
    image.height = height;
    image.src = getSqueredImage(imageUrl, width);
